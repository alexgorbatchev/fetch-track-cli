...
last_modified: 2026-08-14 21:25
status: current
---

# fetch-track-cli Agent Guidelines

Dedicated workspace for `fetch-track`, a Go CLI tool for searching, verifying, downloading, and managing high-fidelity tracks for **Home DJing** equipment and setups. Focused on sourcing full-length Original and Extended Mixes from YouTube and other audio sources, avoiding short radio edits, and inspecting frequency spectrum bandwidth, gain staging, and embedded track metadata with high-res cover art.

## Environment & Target Setup
- **DJ Setup:** Home DJ Controller / Mixer (Engine DJ, Rekordbox, Serato, Pioneer DDJ series).
- **Monitoring Headphones:** Pioneer HDJ-X10 (5 Hz – 40 kHz frequency response for precise beatmatching and EQ control).
- **Track Location & Format:** All master tracks are stored in `./tracks/` in **`.m4a`** format for 100% native album artwork rendering in **Engine DJ** and **macOS Finder**.
- **Go Toolchain & Binary:** Go 1.23+ with Cobra CLI framework (`github.com/spf13/cobra`).

## Primary Single-Command Track Acquisition
To add any new track or YouTube URL in 1 step (runs DJ Full Mix candidate ranker, downloads native M4A, checks DJ quality/bandwidth, resolves 1400x1400 cover art via API fallback, and embeds MP4 `covr` artwork):
```bash
...
- **Artwork Compatibility:** `.m4a` (MP4 container with `covr` atom) is required because **Engine DJ** and **macOS Finder** do NOT render Vorbis picture comments inside `.opus` containers.
...
- **Gain Staging:** Check peak levels and RMS loudness to set appropriate DJ mixer channel trim offsets before playing set.
...
- **Always:** Save all downloaded audio tracks into the `./tracks/` directory as **`.m4a`** files with clean filenames (no `[...]` video IDs) and embedded 1400x1400 `covr` artwork for Engine DJ & Finder.
